function minerals = donarummo_classification(data_table, varargin)
...
% minerals = DONARUMMO_CLASSIFICATION(data_table, FullNames=false)
%
%INPUT
...
%
% FullNames=false - optional name-value pair; will save classifications as
%   mineral abbreviations instead of full names
%
%OUTPUT
...
%
%See also
% eds_classification, msa_classification, kandler_classification, kutuzov_classification, panta_classification, weber_classification

%Updates
% July 2026 - Added FullNames=true optional name-value pair
%
...
% Function code copyright 2026 Austin M. Weber
...
addParameter(inP, 'FullNames', true, @islogical);
...
data_table = parseTableVariableNames(data_table); % Local function, defined at bottom of file

% Classify the mineral for each row of the input table
num_classifications = size(data_table,1); 
minerals = categorical([]);
for n = 1:num_classifications
	mineral = node1(data_table(n,:)); % Begin classification at first node of the sorting scheme tree
	minerals(n) = mineral;
end
